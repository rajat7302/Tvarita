    ...
    <div className="min-h-screen bg-[#FFFDF9] flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-3xl border border-amber-200 shadow-xl max-w-md w-full">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-900">{isForgotPassword ? 'Reset your password' : 'Welcome Back'}</h2>
          <p className="text-xs text-gray-500">{isForgotPassword ? 'Enter your email and we will send a secure reset link.' : 'Sign in to your Tvarita Arts Collective account'}</p>
        </div>

        {error && <div className="mb-4 p-3 bg-red-50 text-red-700 text-xs rounded-xl border border-red-200">{error}</div>}
        {resetMessage && <div className="mb-4 p-3 bg-emerald-50 text-emerald-700 text-xs rounded-xl border border-emerald-200">{resetMessage}</div>}
        ...
        <form onSubmit={isForgotPassword ? handleForgotPassword : handleLogin} className="space-y-4">
          ...
          {!isForgotPassword && (
            <button type="button" onClick={() => { setIsForgotPassword(true); setError(''); }} className="block ml-auto -mt-1 text-xs font-bold text-[#E65100] hover:underline">
              Forgot password?
            </button>
          )}
          ...
          >
            ...
            {loading ? 'Sending...' : isForgotPassword ? 'Send reset link' : 'Sign In'}
          </button>
        </form>
        ...
        {!isForgotPassword && (
          <div className="mt-5">
            <div className="relative mb-4 text-center before:absolute before:inset-x-0 before:top-1/2 before:border-t before:border-amber-100">
              <span className="relative bg-white px-3 text-[10px] font-medium uppercase tracking-wide text-gray-400">or</span>
            </div>
            <GoogleSignInButton onCredential={handleGoogleLogin} onError={setError} />
            <p className="mt-3 text-center text-[10px] text-gray-500">Use the Google email already registered with Tvarita.</p>
          </div>
        )}

        {isForgotPassword && (
          <button type="button" onClick={showLogin} className="mt-5 flex items-center gap-1 mx-auto text-xs font-bold text-gray-600 hover:text-[#E65100]">
            <ArrowLeft className="w-3.5 h-3.5" /> Back to sign in
          </button>
        )}

        {!isForgotPassword && <p className="mt-6 text-center text-xs text-gray-600">
