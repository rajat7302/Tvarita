        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {role === 'artist' && (
            <div className="border-t border-stone-200 pt-4 space-y-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-stone-700 mb-2">Team Members & Roles</label>
                {members.map((m, idx) => (
                  <div key={idx} className="flex gap-2 mb-2">
                    <input
                      ...
                      className="flex-1 p-2 text-xs border border-stone-300 rounded-lg"
                      value={m.memberName}
                    ...
                    <input
                      type="text"
                      required
                      placeholder="Role (e.g. Dhol Performer)"
                      className="flex-1 p-2 text-xs border border-stone-300 rounded-lg"
                      value={m.roleInTeam}
                      onChange={(e) => handleMemberChange(idx, 'roleInTeam', e.target.value)}
                    />
                    {members.length > 1 && (
                      <button type="button" onClick={() => removeMemberField(idx)} className="text-red-500 px-2 font-bold text-xs">✕</button>
                    )}
                  </div>
                ))}
                <button
                  ...
                  className="text-xs bg-stone-100 text-stone-800 font-semibold px-3 py-1.5 rounded-lg border border-stone-300 hover:bg-stone-200"
                >
                  + Add Teammate
                </button>
              </div>
